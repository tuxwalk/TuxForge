...
OUTPUT  = BASE / "releases/tuxforge-0.5-mifi6620l.img"
...
case "${1:-start}" in
    start)
        ...
        echo "TuxForge 0.5" > /tmp/tuxforge-version
        echo "ui-pending" > /tmp/tuxforge-status

        if [ -c /dev/ttyHSL0 ]; then
            echo "=== TuxForge 0.5 ===" > /dev/ttyHSL0
        fi
