...
EARLY_INIT = BASE / "tuxforge-root/init-0.9.1"
OUTPUT  = BASE / "releases/tuxforge-0.9.1-mifi6620l.img"
...
adopt_early_ui()
{
    if [ ! -f /tuxforge-early-ui-pid ]; then
        return 1
    fi

    pid="$(cat /tuxforge-early-ui-pid 2>/dev/null)"
    ...
    {
        echo "TuxForge 0.9.1 UI launcher"
        echo "adopted early tuxui pid=$pid"
    } > /tmp/tuxui-launch.log
...
case "${1:-start}" in
    start)
        ...
        echo "TuxForge 0.9.1" > /tmp/tuxforge-version
        echo "ui-pending" > /tmp/tuxforge-status

        if [ -c /dev/ttyHSL0 ]; then
            echo "=== TuxForge 0.9.1 ===" > /dev/ttyHSL0
        fi

        #
        # Preferred 0.9.1 path:
        # tuxui was launched before Novatel init and is already alive.
        #
        if adopt_early_ui; then
            exit 0
        fi

        #
        # Recovery fallback:
        # if early tuxui died, launch a fresh copy without blocking boot.
        #
        (
            echo "TuxForge 0.9.1 UI launcher"
            echo "early tuxui unavailable; using fallback"
            ...
            if kill -0 "$pid" 2>/dev/null; then
                echo "ready" > /tmp/tuxforge-status
                echo "fallback tuxui ready pid=$pid"
            else
                echo "ui-failed" > /tmp/tuxforge-status
                echo "ERROR: fallback tuxui exited"
            fi
...
def main():
    ...
    # TuxForge 0.9.1 first-stage PID1 wrapper.
