...
/*
 * Real stock-OS handoff for TuxForge boot image.
 *
 * ARM EABI:
 *   execve = 11
 *   chdir  = 12
 ...
 *   chroot = 61
 */
#define TB_SYS_EXECVE 11
#define TB_SYS_CHDIR  12
#define TB_SYS_MOUNT  21
#define TB_SYS_CHROOT 61

#define TB_MS_MOVE 8192

static long tb_mount(const char *src,
                     const char *dst,
                     const char *type,
                     unsigned long flags,
                     const void *data)
{
    register long r0 asm("r0") = (long)src;
    register long r1 asm("r1") = (long)dst;
    register long r2 asm("r2") = (long)type;
    register long r3 asm("r3") = (long)flags;
    register long r4 asm("r4") = (long)data;
    register long r7 asm("r7") = TB_SYS_MOUNT;
...
static long tb_chdir(const char *path)
{
    register long r0 asm("r0") = (long)path;
    register long r7 asm("r7") = TB_SYS_CHDIR;
...
static long tb_chroot(const char *path)
{
    register long r0 asm("r0") = (long)path;
    register long r7 asm("r7") = TB_SYS_CHROOT;
...
static long tb_execve(const char *path,
                      char *const argv[],
                      char *const envp[])
{
    register long r0 asm("r0") = (long)path;
    register long r1 asm("r1") = (long)argv;
    register long r2 asm("r2") = (long)envp;
    register long r7 asm("r7") = TB_SYS_EXECVE;
...
static void reboot_normal(int fbfd)
{
    ...
    last_event = "BOOTING STOCK";
    redraw(fbfd);

    led_off();

    /*
     * mtdblock15 is the stock YAFFS2 system/root partition.
     */
    if (tb_mount("/dev/mtdblock15",
                 "/newroot",
                 "yaffs2",
                 0,
                 0) < 0) {
        last_event = "BOOT: MOUNT FAILED";
        redraw(fbfd);
        return;
    }

    if (tb_chdir("/newroot") < 0) {
        last_event = "BOOT: CHDIR FAILED";
        redraw(fbfd);
        return;
    }

    /*
     * Replace the initramfs root mount with the stock YAFFS2 root.
     */
    if (tb_mount(".", "/", 0, TB_MS_MOVE, 0) < 0) {
        last_event = "BOOT: MOVE FAILED";
        redraw(fbfd);
        return;
    }
    ...
    /*
     * PID 1 remains PID 1 across execve().
     */
    tb_execve("/sbin/init", argv, envp);
