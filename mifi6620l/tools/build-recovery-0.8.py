...
EARLY_INIT = BASE / "tuxforge-root/init-0.8"
OUTPUT  = BASE / "releases/tuxforge-0.8-mifi6620l.img"
...
case "${1:-start}" in
    start)
        ...
        echo "TuxForge 0.8" > /tmp/tuxforge-version
        echo "ui-pending" > /tmp/tuxforge-status

        if [ -c /dev/ttyHSL0 ]; then
            echo "=== TuxForge 0.8 ===" > /dev/ttyHSL0
        fi
...
def main():
    ...
    # TuxForge 0.8 first-stage PID1 wrapper.
