...
EARLY_INIT = BASE / "tuxforge-root/init-0.9"
OUTPUT  = BASE / "releases/tuxforge-0.9-mifi6620l.img"
...
case "${1:-start}" in
    start)
        ...
        echo "TuxForge 0.9" > /tmp/tuxforge-version
        echo "ui-pending" > /tmp/tuxforge-status

        if [ -c /dev/ttyHSL0 ]; then
            echo "=== TuxForge 0.9 ===" > /dev/ttyHSL0
        fi
...
def main():
    ...
    # TuxForge 0.9 first-stage PID1 wrapper.
