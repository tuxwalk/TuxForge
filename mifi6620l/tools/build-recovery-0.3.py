...
OUTPUT  = BASE / "releases/tuxforge-0.3-mifi6620l.img"
...
case "${1:-start}" in
    start)
        ...
        echo "TuxForge 0.3" > /tmp/tuxforge-version
        echo "starting" > /tmp/tuxforge-status

        if [ -c /dev/ttyHSL0 ]; then
            echo "=== TuxForge 0.3 ===" > /dev/ttyHSL0
        fi
...
def main():
    print("=== TuxForge 0.3 builder ===")

    for p in (STOCK, FBFLUSH, SPLASH, TUXREBOOT):
        if not p.exists():
            die(f"missing {p}")
