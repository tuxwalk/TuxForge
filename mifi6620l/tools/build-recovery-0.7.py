...
OUTPUT  = BASE / "releases/tuxforge-0.7-mifi6620l.img"
...
case "${1:-start}" in
    start)
        ...
        echo "TuxForge 0.7" > /tmp/tuxforge-version
        echo "ui-pending" > /tmp/tuxforge-status

        if [ -c /dev/ttyHSL0 ]; then
            echo "=== TuxForge 0.7 ===" > /dev/ttyHSL0
        fi
...
def main():
    ...
    # TuxForge 0.7 first-stage PID1 wrapper.
