...
EARLY_INIT = BASE / "tuxforge-root/init-1.0"
OUTPUT  = BASE / "releases/tuxforge-1.0-mifi6620l.img"
...
case "${1:-start}" in
    start)
        ...
        echo "TuxForge 1.0" > /tmp/tuxforge-version
        echo "ui-pending" > /tmp/tuxforge-status

        if [ -c /dev/ttyHSL0 ]; then
            echo "=== TuxForge 1.0 ===" > /dev/ttyHSL0
        fi
...
def main():
    ...
    # TuxForge 1.0 first-stage PID1 wrapper.
